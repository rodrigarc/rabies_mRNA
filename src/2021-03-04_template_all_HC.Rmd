...
ifelse(isFALSE(dir.exists(paste0("../",result.dir,"chromatrogram"))), dir.create(paste0("../",result.dir),recursive = TRUE),"Result directory for today exists already!")
...
sf <- summarise.quality("../data/rabies_VH_ab1_files/",                           
...
sf$summaries <- sf$summaries %>%
  ...
  mutate(chain = "HC") %>% 
  mutate(ID = str_replace_all(str_extract(file.name, "_I[0-9][0-9]_|-I[0-9][1-9]_"), "_|-","")) %>%
  mutate(well_number = sub("_","",gsub("[[:upper:]]","",well)),
             well_letter = sub("_","",gsub("[[:digit:]]","",well)),
             well = paste0(well_letter,sprintf("%02s", well_number))) %>%
  mutate(file_ID = sub("\\.ab1","",file.name))
...
sf_filtered_csv %>% select(file_ID, ID, plate, well, chain, raw.mean.quality, trimmed.mean.quality,sec.peak.CDR3, sequence) %>% write.csv(paste0("../",result.dir,"sanger_filtered.csv"), row.names = FALSE)
...
ape::write.dna(sangerbasecallfasta, 
          file = paste0("../",result.dir,"HC_all.fasta"), 
          format = 'fasta', 
          nbcol = -1, 
          colsep = "",
          colw = 10000000,
...
rmarkdown::render("2021-03-04_template_animal.Rmd",  # file 2
                  output_file =  paste("report_", i, '_', Sys.Date(), ".html", sep=''), 
                  output_dir = paste0("../",result.dir, Sys.Date()))
}
```
