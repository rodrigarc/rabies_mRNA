...
## performing Dunn's test as post-hoc to keep consistent the statical choice across the paper
ls_for_dunns <- list(group_1 = merged_df$V_SHM[merged_df$.id == "group_1"],
                     group_2 = merged_df$V_SHM[merged_df$.id == "group_2"],
                     group_3 = merged_df$V_SHM[merged_df$.id == "group_3"])
dunn_result <- dunn.test(x = ls_for_dunns, method = "none", kw = TRUE)
#adjusting p-value for comparions to control group using Benjamini-Hochberg's method.
p.adjust(dunn_result$P[2:3], method = "BH")
...
df_heatmap_Vfamily <- merged_info %>% 
     mutate(J_gene = sub("\\*.*","",J_gene),
           V_gene = sub("-.*","", V_gene)) %>%
    dplyr::count(V_gene, J_gene, Group) %>%
    mutate(freq = n/sum(n)*100)
...
expand.grid(sub("-.*","",names_v),sub("\\*.*","",names_j), c("Group 1", "Group 2", "Group 3")) %>%
  ...
  tidyr::replace_na(list(freq = 0, n = 0)) %>%
  mutate(V_gene_family = substr(V_gene, 1,5)) %>%
  ggplot(aes(V_gene_family, J_gene, fill = freq)) +
  geom_tile(color = "black") +
  scale_fill_viridis_c(option = "viridis", direction = 1)+
  theme(axis.text.x = element_text(angle = 90, size = 7, hjust = 1, vjust = 0.5, face = "bold", colour = "black"),
        axis.text.y = element_text(face = "bold", colour = "black"),
...

{
  ...
  for (i in seq_along(color_groups_tree)){
    ...
    if(i %in% c(1:3)){
      tree_new <- ggtree(trees[[1]], aes(color = .id),layout = "ape") %<+% rabies_df +
        geom_tiplab(aes(label = mAb), hjust = -.1) + 
      ...
      tree_new <- ggtree(trees[[1]], aes(color = V_gene_family),layout = "ape") %<+% rabies_df +
